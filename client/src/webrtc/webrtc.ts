...
let myUsername = '';
let targetUsername = '';      // To store username of other peer
...
function sendToServer(msg: { type: string, [a: string]: any }) {
...
function connect() {
  ...
  connection.onmessage = function (evt: { data: string }) {
...
function handleTrackEvent(event: { streams: readonly unknown[] }) {
  log("*** Track event");
...
async function invite(clickedUsername: string) {
...
async function handleVideoOfferMsg(msg: { name: string; sdp: RTCSessionDescriptionInit }) {
...
async function handleVideoAnswerMsg(msg: { sdp: RTCSessionDescriptionInit }) {
...
async function handleNewICECandidateMsg(msg: { candidate: RTCIceCandidateInit }) {
